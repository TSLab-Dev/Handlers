...
namespace TSLab.Script.Handlers
{
    ...
    public sealed class Cutter : IBar2BarHandler, IContextUses
    {
        public IContext Context { get; set; }
        ...
        {
            var oldBars = source.Bars;
            var barsCount = oldBars.Count;
            var newBars = Context?.GetArray<IDataBar>(barsCount) ?? new IDataBar[barsCount];
            var coef = Math.Pow(10, Decimals);

            for (var i = 0; i < barsCount; i++)
            {
                var oldBar = oldBars[i];
                var newBar = !(oldBar is IBar bar)
                    ? new DataBar()
                    : new BidAskBar
                    {
                        Ask = Math.Ceiling(bar.Ask * coef) / coef,
                ...
                newBar.Date = oldBar.Date;
                newBar.Open = Math.Ceiling(oldBar.Open * coef) / coef;
                newBar.High = Math.Ceiling(oldBar.High * coef) / coef;
                newBar.Low = Math.Ceiling(oldBar.Low * coef) / coef;
                newBar.Close = Math.Ceiling(oldBar.Close * coef) / coef;
                newBar.Volume = oldBar.Volume;
                newBar.Interest = oldBar.Interest;
                newBars[i] = newBar;
            }
            return source.CloneAndReplaceBars(newBars);
        }
    }
    ...
    public sealed class SecMultiply : IBar2BarHandler, IContextUses
    {
        public IContext Context { get; set; }
        ...
        public ISecurity Execute(ISecurity source)
        {
            ...
            for (var i = 0; i < barsCount; i++)
            {
                ...
                newBar.Date = oldBar.Date;
                newBar.Open = Math.Ceiling(oldBar.Open * Coef);
                newBar.High = Math.Ceiling(oldBar.High * Coef);
                newBar.Low = Math.Ceiling(oldBar.Low * Coef);
                newBar.Close = Math.Ceiling(oldBar.Close * Coef);
                newBar.Volume = oldBar.Volume;
                newBar.Interest = oldBar.Interest;
                newBars[i] = newBar;
            }
    ...
    public sealed class SecMultiplyWith : IBarDouble2BarHandler, IContextUses
    {
        public IContext Context { get; set; }
        ...
        public ISecurity Execute(ISecurity source, IList<double> src2)
        {
            ...
            for (var i = 0; i < barsCount; i++)
            {
                ...
                var newBar = !(oldBar is IBar bar)
                    ...
                    : new BidAskBar
                    {
                        Ask = Math.Ceiling(bar.Ask * coef) / dcf,
                ...
                newBar.Date = oldBar.Date;
                newBar.Open = Math.Ceiling(oldBar.Open * coef) / dcf;
                newBar.High = Math.Ceiling(oldBar.High * coef) / dcf;
                newBar.Low = Math.Ceiling(oldBar.Low * coef) / dcf;
                newBar.Close = Math.Ceiling(oldBar.Close * coef) / dcf;
                newBar.Volume = oldBar.Volume;
                newBar.Interest = oldBar.Interest;
                newBars[i] = newBar;
            }
    ...
    public sealed class SecDivideWith : IBarDouble2BarHandler, IContextUses
    {
        public IContext Context { get; set; }
