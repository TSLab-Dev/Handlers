...
namespace TSLab.Script.Handlers.Options
{
    ...
    public sealed class Heartbeat : BaseContextHandler, IStreamHandler, IValuesHandlerWithNumber
    {
        private long m_id;
        ...
        public IList<double> Execute(IOption opt) => Execute(opt.UnderlyingAsset);
        public IList<double> Execute(IOption opt, IList<bool> condition) => Execute(opt.UnderlyingAsset, condition);
        public double Execute(IOption opt, bool condition, int index) => Execute(opt.UnderlyingAsset, condition, index);

        public IList<double> Execute(IOptionSeries optSer) => Execute(optSer.UnderlyingAsset);
        public IList<double> Execute(IOptionSeries optSer, IList<bool> condition) => Execute(optSer.UnderlyingAsset, condition);
        public double Execute(IOptionSeries optSer, bool condition, int index) => Execute(optSer.UnderlyingAsset, condition, index);
        ...
        public IList<double> Execute(ISecurity sec, IList<bool> condition)
        {
            // PROD-5496 - В режиме оптимизации отключаюсь
            if (Context.IsOptimization)
                return Constants.EmptyListDouble;

            if (condition.LastOrDefault())
                Recalculate(sec);

            return GetResults();
        }

        public double Execute(ISecurity sec, bool condition, int index)
        {
            // PROD-5496 - В режиме оптимизации отключаюсь
            if (Context.IsOptimization)
                return 0;
        ...
        {
            if (state is CallState callState)
            {
                ...
                if (callState.OnlyAtTradingSession)
                {
                    ...
                    context?.Recalc(RecalcReasons.Heartbeat, callState.DataSourceSecurity);
                }
                else
                    context?.Recalc(RecalcReasons.Heartbeat, callState.DataSourceSecurity);
            }
        ...
        private IList<double> GetResults()
        {
            var barsCount = Context.BarsCount;
            var results = Context.GetArray<double>(barsCount);

            if (barsCount > 0)
                results[barsCount - 1] = m_id;
        ...
        private sealed class CallState
        {
            ...
            /// </summary>
            public readonly SecInfo SecInfo;
            ...
            public CallState(long id, IContext context, IDataSourceSecurity dataSourceSecurity, bool onlyAtTradingSession)
            {
                ...
                CallId = id;
                CallContext = context;
                SecInfo = new SecInfo(dataSourceSecurity);
                OnlyAtTradingSession = onlyAtTradingSession;
                DataSourceSecurity = dataSourceSecurity;
            }

            public IDataSourceSecurity DataSourceSecurity { get; private set; }
        }
        /// <summary>
