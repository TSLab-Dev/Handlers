...
namespace TSLab.Script.Handlers
{
    ...
    public sealed class LastClosedNamePositionExitTime : IBar2ValueDoubleHandler
    {
        ...
        public double Execute(ISecurity source, int barNum)
        {
            ...
            if (pos != null && pos.IsActiveForBar(barNum))
            {
                pos = null;
            }
            var dt = pos?.ExitBar.Date ?? DateTime.MinValue;
            return dt.Hour * 10000.0 + dt.Minute * 100.0 + dt.Second;
        }
    }
    ...
    [HandlerCategory(HandlerCategories.Position)]
    [HelperName("Exit price of last closed named position", Language = Constants.En)]
    [HelperName("Цена выхода последней закрытой позиции по имени", Language = Constants.Ru)]
    [InputsCount(1)]
    [Input(0, TemplateTypes.SECURITY, Name = Constants.SecuritySource)]
    [OutputsCount(1)]
    [OutputType(TemplateTypes.DOUBLE)]
    [Description("Цена выхода последней закрытой позиции по имени.")]
    [HelperDescription("The exit price of the latest closed named position.", Constants.En)]
    public sealed class LastClosedNamePositionExitPrice : IBar2ValueDoubleHandler
    {
        /// <summary>
        /// \~english Close signal name
        /// \~russian Имя сигнала закрытия
