...
namespace TSLab.Script.Handlers
{
    ...
    public sealed class HoldSignalForNBars : IStreamHandler, IValuesHandlerWithNumber, IOneSourceHandler, IBooleanReturns, IBooleanInputs
    {
        private int m_lastTrueIndex = -1;
        ...
        public bool Execute(bool value, int barIndex)
        {
            if (value)
            {
                m_lastTrueIndex = barIndex;
                return true;
            }
            return barIndex >= m_lastTrueIndex && barIndex <= m_lastTrueIndex + NBars;
        }
    }
}
