...
{
    /// <summary>
    /// \~english Sell many options while position risk is 'small' (strike range is set in units of delta)
    /// \~region Продажа группы опционов до заданного уровня риска (диапазон страйков задается в единицах дельты)
    /// </summary>
    [HandlerCategory(HandlerCategories.OptionsPositions)]
    [HelperName("Sell option group (delta range)", Language = Constants.En)]
    [HelperName("Продажа группы опционов (диапазон по дельте)", Language = Constants.Ru)]
    [HandlerAlwaysKeep]
    [InputsCount(9)]
    ...
    [Description("Продажа группы опционов до заданного уровня риска (диапазон страйков задается в единицах дельты)")]
    [HelperDescription("Sell many options while position risk is 'small' (strike range is set in units of delta)", Constants.En)]
    public class SellOptionGroupDelta : IContextUses, IValuesHandlerWithNumber
    {
