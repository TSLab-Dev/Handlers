...
namespace TSLab.Script.Handlers
{
    ...
    public sealed class EMA : DoubleStreamAndValuesHandlerWithPeriod
    {
        ...
        public override bool IsGapTolerant => IsSimple;
        ...
        {
            m_lastResult = index > 0 ? m_k * (source - m_lastResult) + m_lastResult : source;
            return m_lastResult;
        }
    }
    ...
    {
        private readonly Func<double, double, bool> m_switchFunc;
        private ShrinkedList<double> m_source;
        private int m_lastIndex = -1;
        private double m_lastValue = double.NaN;

        protected HighestLowest(Func<double, double, bool> switchFunc)
        {
            m_switchFunc = switchFunc ?? throw new ArgumentNullException(nameof(switchFunc));
        }

        public override bool IsGapTolerant => true;
        ...
        protected override double Execute()
        {
            ...
            if (m_source.Count == Period)
            {
                if (m_lastIndex > 0)
                    m_lastIndex--;
                else
                    m_lastValue = m_source[1];
            }
            else if (m_source.Count == 0)
            {
                m_lastIndex = 0;
                m_lastValue = m_executeContext.Source;
            }
            m_source.Add(m_executeContext.Source);
            var iMax = Math.Min(m_source.Count, Period);

            for (var i = m_lastIndex + 1; i < iMax; i++)
            {
                var value = m_source[i];
                if (m_switchFunc(m_lastValue, value))
                {
                    m_lastIndex = i;
                    m_lastValue = value;
                }
    ...
    {
        private static readonly Func<double, double, bool> s_switchFunc = (oldValue, newValue) => oldValue <= newValue;
